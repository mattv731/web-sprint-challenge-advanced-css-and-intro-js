...
for(let i = 0; i < artists.length; i++){
  if (artists[i].name.includes("Vincent van")){
...
function get20s(array) {
  const lastCenturyArtists = [];
  for(let i = 0; i < array.length; i++){
    const year = array[i].years.split(' - ');
    if (year[0] >= 1900){
      lastCenturyArtists.push(array[i].name)
    }
  }
...
function addArtist(array) {
  array.push({ 
    id: 20,
    name: "Matthew Von Gunten", 
    years: 1999 - 2021,
    genre: "Web Design", 
    nationality: "American",
    bio: "I like working on cars and making websites!",})
  return array
}
